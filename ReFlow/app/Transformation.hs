...
module Transformation
  ( transformProgramSymb,
    isTauMinus,
    isTauPlus,
    ErrVarEnv,
    findOrigFunInProg,
    findOrigFunInSpec,
    findOrigFunInRealProg,
    lookupOrigFun,
    -- TODO: Used only for testing. Make sure they are used in other modules too
    TranStateInterp,
    TransformationState(..),
    FreshErrVar(..),
    betaBExprStm,
...
betaBExprStm rprog isTPlus decls f be check (BIte fbexpr thenStm elseStm) = do
...
generateVarName ae environment counter be =
  case findInEnv ae environment of
    Just (v,_,_)  -> (v,environment,counter)
...
betaPlusVar _ g pathCond decls (FEPred isTrans _ f args) = do
  currentStateEnv  <- get
  let fCurrentState = findFreshErrsInTranStateInterp f currentStateEnv
...
betaMinusVar _ g pathCond decls (FEPred isTrans _ f args) = do
  currentStateEnv  <- get
  let fCurrentState = findFreshErrsInTranStateInterp f currentStateEnv
