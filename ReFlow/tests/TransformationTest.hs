...
returnsValueEqualTo' lhs rhs = return (fst $ runState lhs initialState) `returnsValueEqualTo` rhs
  where
    initialState = [("f", TransState { freshErrVar = FreshErrVar { env = [], count = 0, localEnv = [] },
                                                     forExprMap = [] })]
